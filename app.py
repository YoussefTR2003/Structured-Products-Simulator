...
def fetch_market_params(tickers: list[str], lookback_years: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    """Fetch market data and compute parameters."""
...
with col2:
    ...
    ax.hist(payoff, bins=80, edgecolor='black', alpha=0.7)
    ...
    if calls.size:
        ax2.hist(calls, bins=np.arange(0, len(obs_idx) + 1) - 0.5, edgecolor='black', alpha=0.7)
        ax2.set_xlabel("Observation number (0-based)")
        ax2.set_ylabel("Count")
        ax2.grid(True, alpha=0.3)
    else:
        ax2.text(0.5, 0.5, "No autocalls in this run.", ha="center", va="center", fontsize=14)
        ax2.axis("off")
    st.pyplot(fig2)
    plt.close()
    ...
    for i in range(m):
        ...
        ax3.plot(tgrid, ratio_line, linewidth=0.8, alpha=0.6)
    ax3.axhline(coupon_trigger, linestyle="--", color='green', label=f'Coupon trigger ({coupon_trigger})', linewidth=1.5)
    ax3.axhline(call_trigger, linestyle="--", color='blue', label=f'Autocall trigger ({call_trigger})', linewidth=1.5)
    ax3.axhline(barrier, linestyle="--", color='red', label=f'Barrier ({barrier})', linewidth=1.5)
    ax3.set_xlabel("Time (years)")
    ax3.set_ylabel("Basket ratio")
    ax3.legend(loc='best')
